    ...
    <section className="relative w-full overflow-hidden bg-surface px-margin-mobile py-10 md:px-margin">
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 lg:grid-cols-12 lg:gap-gutter">
        ...
        <div className="flex max-w-2xl flex-col items-start lg:col-span-8">
          <div className="mb-6 inline-flex items-center gap-1.5 rounded-full bg-surface-container-high px-2 py-0.5 font-label text-label-sm font-medium tracking-wider text-primary uppercase">
            <span className="material-symbols-outlined text-[15px]">verified</span>
            Canal Preferente para Doctores y Clínicas
          </div>

          <h1 className="mb-4 font-serif text-headline-lg-mobile text-on-surface md:text-headline-lg">
            Consulte con Nuestros <span className="text-primary italic">Maestros Ceramistas</span> y
            Especialistas CAD/CAM.
          </h1>

          <p className="mb-6 max-w-2xl font-sans text-body-md text-on-surface-variant md:text-body-lg">
            Obtenga asesoría técnica de alta fidelidad, valoración de viabilidad anatómica y
            presupuesto por correo electrónico en{' '}
            <span className="font-medium text-secondary">menos de 2 horas hábiles</span>. Sin
            compromiso ni plantillas genéricas.
          </p>
          ...
          <div className="grid w-full grid-cols-1 gap-2 rounded-sm bg-surface-container-low p-2 sm:grid-cols-3">
            {metrics.map(({ icon, value, label }) => (
              <div key={label} className="flex flex-col py-1">
                <span className="flex items-center gap-1.5 font-label text-label-lg font-medium text-primary">
          ...
          <div className="overflow-hidden rounded-xl bg-surface-container-low p-2 shadow-xl">
            <div className="relative h-64 w-full overflow-hidden rounded-sm">
              ...
              <div className="pointer-events-none absolute inset-x-3 bottom-3 flex items-center justify-between">
                <span className="rounded-sm bg-surface-container-lowest/80 px-2 py-1 font-label text-label-sm font-semibold tracking-wider text-primary uppercase backdrop-blur-md">
                  Atelier Anterior
                </span>
                <span className="font-label text-label-sm font-semibold tracking-wider text-on-surface uppercase">
